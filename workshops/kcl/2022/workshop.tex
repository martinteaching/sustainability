...
\begin{enumerate}
    ...
    \emph{Note: if you receive a permissions error when running this command on a Linux machine, you may be required to run the command as the root user by prefixing it with the \mintinline[bgcolor=bg]{bash}{sudo} keyword. To permanently allow a non-root user access to Docker you can add a user (e.g. `user') to the docker Unix group with the command \mintinline[bgcolor=bg]{bash}{sudo usermod -aG docker user}.}
